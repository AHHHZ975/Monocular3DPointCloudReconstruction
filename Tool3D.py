...
def meshesToRGBs(meshPath, imageName, width, height, numberOfViews, view, visible=False):
    """
    ...
    if visible:
        ...
        # Look at the origin from the front (along the -Z direction, into the screen), with Y as Up.
        center = [0, 0, 0]  # look_at target
        eye =  [0.8, 0.8, ((-1)**(view))*(view*2/numberOfViews)]  # camera position
        up = [0, 1, 0]  # camera orientation
...
def generateData(srcPath, destPath, width, height, numberOfData, numberOfViews, visible=False, train=False):
    """
    ...
    for i, shapeNetPath in enumerate(shapeNetPaths):        
        ...
        for j in range(numberOfViews):
            Path(f'{destPath}/{counter}').mkdir(parents=True, exist_ok=True)
            imagePath = f'{cfg.ROOT_DIR}/{destPath}/{counter}/{str(counter)}.jpg'
            print(shapeNetPath)
            meshesToRGBs(shapeNetPath, imagePath, width, height, numberOfViews, j, visible)

            # Generate labels (pointclouds) (y)
            pointCloud = meshesToPointClouds(shapeNetPath, cfg.SAMPLE_SIZE)
            
            # Save pointclouds as .ply files        
            pointCloudPath = f'{cfg.ROOT_DIR}/{destPath}/{counter}/{str(counter)}.ply'
            o3d.io.write_point_cloud(pointCloudPath, pointCloud)
        
            # Save pointclouds as .txt files
            xyzPath = f'{cfg.ROOT_DIR}/{destPath}/{counter}/{str(counter)}.txt'        
...
def main():
    ...
    sourcePath = 'C:/Users/AmirHossein/OneDrive/Desktop/ShapeNet'
    trainData = generateData(sourcePath, destinationPath, cfg.IMAGE_WIDTH, cfg.IMAGE_HEIGHT, 470, 4, train=True, visible=False)
    testData = generateData(sourcePath, destinationPath, cfg.IMAGE_WIDTH, cfg.IMAGE_HEIGHT, 28, 4, train=False, visible=False)
